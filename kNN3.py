...
def plot_roc(y_true, y_estimate, threshold):
	...
	plt.savefig('plot/knn_roc_' + str(threshold) + '.png')
	plt.clf()

if __name__ == "__main__":
	threshold = [2.5, 3, 3.5, 4]
	new_path = "ml-latest-small/ratings_new.csv"
	  
	file_path = os.path.expanduser(new_path)
	reader = Reader(sep=',')
	...
	for th in threshold:
		algo = KNNWithMeans(k=14, sim_options=sim_options)
		algo.fit(trainset)
